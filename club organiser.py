...
class SeniorClubOuting:
    def __init__(self, master):
        ...
        self.max_capacity = 0  # Maximum capacity based on current plan
    ...
    def calculate_cost(self):
        try:
            ...
            # Calculate remaining seats based on the maximum capacity of the current plan
            remaining_seats = self.max_capacity - self.num_senior_citizens - self.num_carers
            ...
            window_width = 800
            window_height = 100 + (self.max_capacity) * 30
            ...
            submit_button.grid(row=self.max_capacity, columnspan=4)
            ...
            calculate_profit_button.grid(row=self.max_capacity + 1, columnspan=4)
            ...
            if remaining_seats > 0:
                ...
                self.extra_passengers_button.grid(row=self.max_capacity + 2, columnspan=4)
    ...
    def add_extra_passengers(self):
        ...
        # Calculate remaining seats based on current plan
        remaining_seats = self.max_capacity - self.num_senior_citizens - self.num_carers

        # Adjust window size based on content
        window_width = 800
        window_height = 100 + (remaining_seats * 30) + 50
        extra_passenger_window.geometry(f"{window_width}x{window_height}")

        label = tk.Label(extra_passenger_window, text=f"Available Seats: {remaining_seats}")
        ...
        submit_button.grid(row=remaining_seats+1, columnspan=4)
